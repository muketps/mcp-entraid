...
class ResetMfaWorkflow:
    ...
    async def start(self, user_upn: str) -> ResetMfaStartResponse:
        ...
        return ResetMfaStartResponse(
            ...
            confirmation_message=(
                f"O usuario {user_display_name or user_principal_name or user_upn} possui "
                f"{len(phone_methods)} metodo(s) de telefone e {len(mfa_methods)} metodo(s) "
                "Microsoft Authenticator/MFA. Deseja continuar e remover esses metodos?"
            ),
            expires_at=expires_at,
            request_id=None,
        )
    ...
    async def _execute_delete_authentication_methods(
        ...
        state.current_step = STEP_REVOKE_SESSIONS
        self._workflow_store.save_reset_mfa_state(state)

        success = len(failed_methods) == 0
        message = "Metodos de telefone e MFA processados."
        if failed_methods:
            message = "Metodos de telefone e MFA processados com falhas parciais."
        ...
        return ResetMfaStepResponse(
            ...
            confirmation_required=True,
            confirmation_message=(
                "Metodos de telefone e MFA processados. Deseja revogar as sessoes do usuario "
                "agora para exigir novo login?"
            ),
            status=state.status,
            message=message,
            request_id=None,
